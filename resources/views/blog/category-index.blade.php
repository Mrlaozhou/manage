    ...
    <form class="layui-form x-search">
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline x-box-add-btn">
                <button type="button" class="layui-btn" onclick="openLayer('添加分类','{{ url('blog-category/create') }}')"><i class="layui-icon">&#xe61f;</i>添加分类</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    layui.use( ['table','jquery','layer','element','form'],function () {
        ...
        table.render({
            ...
            // ,width:'100%'
            ,height: 500
            ,url: '{{ route('api.blog.category.index') }}' //数据接口
            ,page: true //开启分页
            ,limit: 100
            ,method: 'post'
            ,loading: true
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'sign', title: '标识', width:'8%'}
                ,{field: 'pid', title: '父级', width:'12%'}
        ...
        table.on( 'tool(current)',function (obj) {
            ...
            if( curr == 'del' )
            {
                layer.confirm( '确定要删除此列吗?',{icon: 3, title:'提示'},function (index) {
                    $.post( "{{ route('api.blog.category.delete') }}", {uuid:data.uuid,_token:_TOKEN}, function(res){
                        if( res.code == 2900 ) {
                            layer.msg('删除成功');
                            table.reload('blogCategoryTable');
                            return;
                        }else{
                ...
                openLayer('编辑-'+data.name,'{{ url('blog-category/update') }}'+'/'+data.uuid);
            }else{
