select `manage_p`.`uuid`, `manage_p`.`route`, `manage_p`.`name`, `manage_p`.`pid`
...
where (`manage_p`.`status` = '1') and `manage_p`.`style` in (1, 3, 5, 7)
GROUP BY uuid
order by `manage_p`.`createdtime` desc
...
select `manage_p`.`uuid`, `manage_rp`.`ruuid`, `manage_ar`.`auuid`, `manage_p`.`name`, `manage_p`.`route`, `manage_r`.`name` as `rname`
from `manage_privilege` as `manage_p`
left join `manage_relation2` as `manage_rp` on `manage_p`.`uuid` = `manage_rp`.`puuid`
left join `manage_role` as `manage_r` on `manage_r`.`uuid` = `manage_rp`.`ruuid`
left join `manage_relation1` as `manage_ar` on `manage_r`.`uuid` = `manage_ar`.`ruuid`
...
select `manage_p`.`uuid`, `manage_rp`.`ruuid`, `manage_ar`.`auuid`, `manage_p`.`name`, `manage_p`.`route`, `manage_p`.`alias` as `palias`, `manage_p`.`mode`, `manage_p`.`pid`, `manage_p`.`type`, `manage_p`.`style` as `pstyle`, `manage_r`.`name` as `rname`
from `manage_privilege` as `manage_p`
left join `manage_relation2` as `manage_rp` on `manage_p`.`uuid` = `manage_rp`.`puuid`
left join `manage_role` as `manage_r` on `manage_r`.`uuid` = `manage_rp`.`ruuid`
left join `manage_relation1` as `manage_ar` on `manage_r`.`uuid` = `manage_ar`.`ruuid`
